...
void normalize_online(double *pr, double *mean, double *var, int n, double t, double decay_factor)
{
    for (int i = 0; i < n; i++)
    {
        ...
        mean[i] = new_m * decay_factor; // Apply decay factor to the mean
        var[i] = new_v * decay_factor;  // Apply decay factor to the variance
...
double compute_anomaly_score(int t, double **pagerank1, double **pagerank2, double **mean, double **var, int n, double decay_factor)
{
    int start_i = 0;
    int end_i = 4;
    ...
    {
        max[i] = -1000;
        normalize_online(d[i], mean[i], var[i], n, t, decay_factor);

        for (int j = 0; j < n; j++)
        {
            if (max[i] < std::abs(d[i][j]))
                max[i] = std::abs(d[i][j]);
        }
        total_max *= max[i];
    }
